...
struct HalfedgeHasher {
    HalfedgeHasher(std::vector<std::array<unsigned int, 3>> const& faces_) : faces(&faces_) {}
...
struct HalfedgeEqualTo {
    HalfedgeEqualTo(std::vector<std::array<unsigned int, 3>> const& faces_) : faces(&faces_) {}
...
TriangleMesh::TriangleMesh(std::vector<Vector3d> const& vertices, std::vector<std::array<unsigned int, 3>> const& faces)
    : m_vertices(vertices), m_faces(faces), m_e2e(3 * faces.size()), m_v2e(vertices.size()) {
    construct();
}

TriangleMesh::TriangleMesh(double const* vertices, unsigned int const* faces, std::size_t nv, std::size_t nf)
    : m_vertices(nv), m_faces(nf), m_e2e(3 * nf), m_v2e(nv) {
