...
Discregrid::DiscreteGrid::DiscreteGrid(const DiscreteGrid& a)
    : m_domain(a.m_domain),
      m_resolution(a.m_resolution),
      m_cell_size(a.m_cell_size),
      m_inv_cell_size(a.m_inv_cell_size),
      m_n_cells(a.m_n_cells),
      m_n_fields(a.m_n_fields) {}
...
AlignedBox3d DiscreteGrid::subdomain(MultiIndex const& ijk) const {
    auto origin = m_domain.min() + Map<Matrix<unsigned int, 3, 1> const>(ijk.data()).cast<double>().cwiseProduct(m_cell_size);
