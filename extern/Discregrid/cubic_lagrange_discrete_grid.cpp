...
[[maybe_unused]]
Matrix<double, 32, 1> shape_function(Vector3d const &xi, Matrix<double, 32, 3> *gradient = nullptr) {
...
Matrix<double, 32, 1> shape_function_(Vector3d const &xi, Matrix<double, 32, 3> *gradient = nullptr) {
    auto res = Matrix<double, 32, 1>{};
...
inline uint64_t zValue(Vector3d const &x, double invCellSize) {
    ...
    std::array<unsigned int, 3> p = {static_cast<unsigned int>(static_cast<int64_t>(key[0]) - (std::numeric_limits<int>::lowest() + 1)),
...
Vector3d CubicLagrangeDiscreteGrid::indexToNodePosition(unsigned int l) const {
    auto x = Vector3d{};

    auto n = Matrix<unsigned int, 3, 1>::Map(m_resolution.data());

    [[maybe_unused]] auto nv = (n[0] + 1) * (n[1] + 1) * (n[2] + 1);
    [[maybe_unused]] auto ne_x = (n[0] + 0) * (n[1] + 1) * (n[2] + 1);
    [[maybe_unused]] auto ne_y = (n[0] + 1) * (n[1] + 0) * (n[2] + 1);
    [[maybe_unused]] auto ne_z = (n[0] + 1) * (n[1] + 1) * (n[2] + 0);
    [[maybe_unused]] auto ne = ne_x + ne_y + ne_z;
...
CubicLagrangeDiscreteGrid::CubicLagrangeDiscreteGrid(AlignedBox3d const &domain, std::array<unsigned int, 3> const &resolution)
    : DiscreteGrid(domain, resolution) {}

CubicLagrangeDiscreteGrid::CubicLagrangeDiscreteGrid(const CubicLagrangeDiscreteGrid &a)
    : DiscreteGrid(a), m_nodes(a.m_nodes), m_cells(a.m_cells), m_cell_map(a.m_cell_map) {}
...
unsigned int CubicLagrangeDiscreteGrid::addFunction(ContinuousFunction const &func, bool verbose, SamplePredicate const &pred) {
...
#pragma omp for schedule(static) nowait
        for (int l = 0; l < static_cast<int>(n_nodes); ++l) {
            ...
            if (verbose && (++counter == n_nodes || duration_cast<milliseconds>(high_resolution_clock::now() - t0).count() > 1000u)) {
                auto res = std::async(std::launch::async, [&]() {
                    ...
                    std::cout << "\r"
                              << "Construction " << std::setw(20) << 100.0 * static_cast<double>(counter) / static_cast<double>(n_nodes) << "%";
                    mutex.unlock();
                });
            }
        }
    ...
    if (verbose) {
        std::cout << "\rConstruction time " << std::setw(15)
                  << static_cast<double>(duration_cast<milliseconds>(high_resolution_clock::now() - t0_construction).count()) / 1000.0 << "s"
                  << std::endl;
    }
...
bool CubicLagrangeDiscreteGrid::determineShapeFunctions(unsigned int field_id, Eigen::Vector3d const &x, std::array<unsigned int, 32> &cell,
                                                        Eigen::Vector3d &c0, Eigen::Matrix<double, 32, 1> &N,
    ...
    if (i_ == std::numeric_limits<unsigned int>::max()) return false;

    auto sd = subdomain(i);
    i = i_;
    [[maybe_unused]] auto d = sd.diagonal().eval();
...
double CubicLagrangeDiscreteGrid::interpolate(unsigned int field_id, Eigen::Vector3d const &xi, const std::array<unsigned int, 32> &cell,
                                              const Eigen::Vector3d &c0, const Eigen::Matrix<double, 32, 1> &N, Eigen::Vector3d *gradient,
                                              Eigen::Matrix<double, 32, 3> *dN) const {
    if (!gradient) {
        auto phi = 0.0;
...
double CubicLagrangeDiscreteGrid::interpolate(unsigned int field_id, Vector3d const &x, Vector3d *gradient) const {
    auto mi = (x - m_domain.min()).cwiseProduct(m_inv_cell_size).cast<unsigned int>().eval();
    if (mi[0] >= m_resolution[0]) mi[0] = m_resolution[0] - 1;
    if (mi[1] >= m_resolution[1]) mi[1] = m_resolution[1] - 1;
    if (mi[2] >= m_resolution[2]) mi[2] = m_resolution[2] - 1;
...
void CubicLagrangeDiscreteGrid::reduceField(unsigned int field_id, Predicate pred) {
    ...
    std::sort(sort_pattern.begin(), sort_pattern.end(), [&](unsigned int i, unsigned int j) { return z_values[i] < z_values[j]; });
    ...
    auto coeffs_ = coeffs;
    std::transform(sort_pattern.begin(), sort_pattern.end(), coeffs.begin(), [&coeffs_](unsigned int i) { return coeffs_[i]; });
}
