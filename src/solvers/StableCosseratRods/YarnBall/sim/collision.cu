...
__global__ void buildCollisionList(MetaData* data, int maxCols, int* errorReturn) {
    ...
    if (std::abs(ids.y - ids.x) <= 2) return;

    // if (!tid) printf("ids: x = %d, y = %d\n", data->d_boundColList[tid].x, data->d_boundColList[tid].y);
    // if (!tid) printf("ids: x = %d, y = %d\n", ids.x, ids.y);
    // if (!tid) printf("BcollList \n");
    ...
    if (d2 < r * r) {
        ...
        if (numCols >= MAX_COLLISIONS_PER_SEGMENT)
            errorReturn[0] = ERROR_MAX_COLLISIONS_PER_SEGMENT_EXCEEDED;
        else
...
__global__ void recomputeStepLimitKernel(MetaData* data) {
    ...
    if (LIMIT && (bool)(data->d_lastFlags[tid] & (uint32_t)VertexFlags::hasNext)) {
        ...
        if (USE_VEL_RADIUS) minDist += length(data->d_dx[tid]);
...
template __global__ void buildAABBs<true>(MetaData* data, int* errorReturn);
template __global__ void buildAABBs<false>(MetaData* data, int* errorReturn);
template __global__ void buildCollisionList<true>(MetaData* data, int maxCols, int* errorReturn);
template __global__ void buildCollisionList<false>(MetaData* data, int maxCols, int* errorReturn);
template __global__ void recomputeStepLimitKernel<true, true>(MetaData* data);
template __global__ void recomputeStepLimitKernel<true, false>(MetaData* data);
template __global__ void recomputeStepLimitKernel<false, true>(MetaData* data);
template __global__ void recomputeStepLimitKernel<false, false>(MetaData* data);
}  // namespace YarnBall