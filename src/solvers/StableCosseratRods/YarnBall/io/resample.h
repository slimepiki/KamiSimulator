...
inline vector<double> resampleCMRCoords(const vector<vec3>& cmr, double start, double end, const double tarSegLen) {
    ...
    if (ts.size() == 1)
        ...
        for (int i = 0; i < N; i++) ts[ts.size() - 2 - i] -= dt * (N - i) / (double)N;
    }
...
#pragma omp section
    ...
    if (forward.size() > 4 * BORDER && backward.size() > 4 * BORDER) {
        ...
        for (uint32_t i = BORDER, j = backward.size() - 1 - BORDER; i < forward.size() - 8; i++) {
            auto t = forward[i];
        ...
        merged.reserve(splitIndex.x + splitIndex.y + 10);
        ...
        for (int i = splitIndex.y - 1; i >= 0; i--) merged.push_back(backward[i]);

        // Blend the split point
        double dt = forward[splitIndex.x] - mid;
        for (int i = 0; i < BORDER; i++) merged[splitIndex.x - i - 1] -= dt * (BORDER - i) / (1. + BORDER);
...
#pragma omp parallel for schedule(static, 1024)
    for (uint32_t i = 0; i < ts.size(); i++) resampled[i] = (vec3)sampleCurve(cmr, ts[i]);
