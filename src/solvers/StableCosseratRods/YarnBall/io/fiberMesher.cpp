...
namespace YarnBall {
inline float randf(fastPRNG::fastXS64& rng) { return glm::clamp((uint32_t)rng.xoroshiro128p() / (float)UINT32_MAX, 0.f, 1.f); }
        ...
        for (int i = 0; i < meta.numVerts; i++) {
            ...
            if (!(v0.flags & (uint32_t)VertexFlags::hasNext)) {
                if (numVerts - vertStart > 2) {
                    fprintf(file, "l");
                    for (size_t i = vertStart; i < numVerts; i++) fprintf(file, " %zd", i + 1);
                    fprintf(file, "\n");
                }
                vertStart = numVerts;
                continue;
            ...
            for (int hi = 0; hi < numHeightSegs; hi++) {
                ...
                Rotor frame = q[1];
                if (t < 0.5f)
                    frame = normalize(mix(q[0].v, frame.v, t + 0.5f));
                else
                    frame = normalize(mix(frame.v, q[2].v, t - 0.5f));
                ...
                if (hi && hi != numHeightSegs) twist += fiberFrizz * (vec3(randf(prng), randf(prng), randf(prng)) * 2.f - 1.f);
