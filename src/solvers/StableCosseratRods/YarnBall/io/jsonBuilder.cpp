...
shared_ptr<Sim> buildFromJSON(std::string path) {
    ...
    if (!file.is_open()) throw std::runtime_error("Error opening file !!!!" + path);

    Json::CharReaderBuilder rbuilder;
    std::string errs;

    Json::Value root;
    if (!Json::parseFromStream(rbuilder, file, &root, &errs)) throw std::runtime_error("Error parsing the JSON file: " + errs);

    shared_ptr<Sim> sim = nullptr;
    ...
    if (!root["transform"].isNull()) {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 4; j++) transform[j][i] = t[i][j].asFloat();
    }
    ...
    if (!simRoot.isNull()) {
        // Material
        if (!simRoot["density"].isNull()) density = simRoot["density"].asFloat();
        if (!simRoot["frictionCoeff"].isNull()) sim->meta.frictionCoeff = simRoot["frictionCoeff"].asFloat();
        if (!simRoot["kStretch"].isNull()) kStretch = simRoot["kStretch"].asFloat();
        if (!simRoot["kBend"].isNull()) kBend = simRoot["kBend"].asFloat();
        if (!simRoot["kCollision"].isNull()) sim->meta.kCollision = simRoot["kCollision"].asFloat();
        if (!simRoot["kFriction"].isNull()) sim->meta.kFriction = simRoot["kFriction"].asFloat();
        if (!simRoot["damping"].isNull()) sim->meta.damping = simRoot["damping"].asFloat();
        ...
        if (!simRoot["drag"].isNull()) sim->meta.velocityDecay = simRoot["drag"].asFloat();

        // Sim params
        if (!simRoot["maxTimeStep"].isNull()) sim->maxH = simRoot["maxTimeStep"].asFloat();
        if (!simRoot["numIterations"].isNull()) sim->meta.numItr = simRoot["numIterations"].asInt();
        if (!simRoot["detectionPeriod"].isNull()) sim->meta.detectionPeriod = simRoot["detectionPeriod"].asInt();
        if (!simRoot["detectionScaler"].isNull()) sim->meta.detectionScaler = simRoot["detectionScaler"].asFloat();
        if (!simRoot["stepLimit"].isNull()) sim->meta.useStepSizeLimit = simRoot["stepLimit"].asBool() ? 1 : 0;
        if (!simRoot["velStepLimit"].isNull()) sim->meta.useVelocityRadius = simRoot["velStepLimit"].asBool() ? 1 : 0;
        if (!simRoot["bvhRebuildPeriod"].isNull()) sim->meta.bvhRebuildPeriod = simRoot["bvhRebuildPeriod"].asFloat();
    }
    ...
    if (!root["glueEndpoints"].isNull()) {
        ...
        if (radius >= 0) sim->glueEndpoints(radius);
    }
    ...
    if (!root["fixBorders"].isNull()) {
        ...
        if (borders.isArray() && borders.size() == 6) {
            float border[6];
            for (int i = 0; i < 6; i++) border[i] = borders[i].asFloat();

            // Get bounding box
            Kit::Bound<> bounds;
            for (int i = 0; i < sim->meta.numVerts; i++) bounds.absorb(sim->verts[i].pos);
            ...
            for (int i = 0; i < sim->meta.numVerts; i++)
                if (!bounds.contains(sim->verts[i].pos)) sim->verts[i].invMass = 0;
        }
    }
                ...
                if (vertices[i].isArray()) {
                    ...
                    if (sphere.size() >= 4) {
                        ...
                        for (int j = 0; j < sim->meta.numVerts; j++)
                            if (glm::length2(sim->verts[j].pos - pos) < r2) sim->verts[j].invMass = 0;
                    } else {
                        ...
                        if (closest >= 0) sim->verts[closest].invMass = 0;
                    }
                } else
                    sim->verts[vertices[i].asInt()].invMass = 0;  // Pin exact vertex index.
        }
    }
...
void Sim::glueEndpoints(float searchRadius) {
    ...
    for (int i = 0; i < meta.numVerts; i++) {
        ...
        if (hasPrev ^ hasNext) endPoints.push_back(i);
    }

#pragma omp parallel for schedule(dynamic, 1)
    for (uint32_t k = 0; k < endPoints.size(); k++) {
        int i = endPoints[k];
