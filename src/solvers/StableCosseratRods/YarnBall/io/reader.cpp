...
shared_ptr<Sim> createFromCurves(vector<vector<glm::vec3>>& curves, vector<bool>& isCurveClosed, int numVerts) {
    shared_ptr<Sim> sim = std::make_shared<Sim>(numVerts);
    numVerts = 0;
    float maxLen = 0;
...
shared_ptr<Sim> readFromBCC(std::string path, float targetSegLen, mat4 transform, bool breakUpClosedCurves, bool allowResample) {
    BCCHeader header;
    FILE* pFile = fopen(path.c_str(), "rb");
    if (!pFile) throw std::runtime_error("Could not open file" + path);
    [[maybe_unused]]
    auto res = fread(&header, sizeof(header), 1, pFile);
    ...
    if (!isPolyline && (header.curveType[0] != 'C' || header.curveType[1] != '0')) throw std::runtime_error("Not polyline or uniform CMR spline");
    if (header.dimensions != 3) throw std::runtime_error("Only curves in 3D are supported");
    ...
    for (size_t i = 0; i < header.curveCount; i++) {
        int numPoints;
        res = fread(&numPoints, sizeof(int), 1, pFile);
        bool isClosed = numPoints < 0;
        numPoints = abs(numPoints);

        vector<vec3> points(numPoints);
        res = fread(&points[0], sizeof(vec3), numPoints, pFile);
...
shared_ptr<Sim> readFromOBJ(std::string path, float targetSegLen, mat4 transform, bool breakUpClosedCurves, bool allowResample) {
    ifstream file(path);
    ...
    for (auto& line : lines) {
        ...
        for (int index : line) curve.push_back(vertices[index - 1]);
        ...
        if (allowResample) curve = Resample::resampleCMR(curve, 1, curve.size() - 2, targetSegLen);
...
shared_ptr<Sim> readFromPoly(std::string path, float targetSegLen, mat4 transform, bool breakUpClosedCurves, bool allowResample) {
    ...
    while (std::getline(file, line)) {
        ...
        if (isPoint) {
            ...
            if (iss >> id >> colon >> x >> y >> z && colon == ':') {
                ...
                if (id != points.size()) throw std::runtime_error("Error parsing .poly. Point id mismatch.");
            } else
                throw std::runtime_error("Error parsing .poly. Point parse error.");
        } else {
            int id, i, j;
            char colon;
            if (iss >> id >> colon >> i >> j && colon == ':')
                ...
                throw std::runtime_error("Error parsing .poly. Segment parse error.");
        }
    }
    ...
    for (uint32_t i = 0; i < points.size(); i++) {
        ...
        if (numCon > 2) throw std::runtime_error("Graphs not allowed.");
    ...
    for (uint32_t i = 0; i < points.size(); i++) {
        ...
        while (true) {
            ...
            for (auto it = range.first; it != range.second; it++) {
                ...
                if ((uint32_t)it->second == i) isEnd = true;
            }
