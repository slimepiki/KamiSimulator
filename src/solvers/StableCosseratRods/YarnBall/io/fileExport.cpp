...
void Sim::exportToBCC(std::string path, bool exportAsPolyline) {
    ...
    for (auto seg : segs) header.totalControlPointCount += seg.y - seg.x + 1;
    ...
    for (auto seg : segs) {
        ...
        for (int i = seg.x; i <= seg.y; i++) fwrite(&verts[i].pos, sizeof(vec3), 1, pFile);
    }
...
void Sim::exportToOBJ(std::string path) {
    ...
    for (int i = 0; i < meta.numVerts; i++) fprintf(pFile, "v %.16f %.16f %.16f\n", verts[i].pos.x, verts[i].pos.y, verts[i].pos.z);
    fprintf(pFile, "\n# Curves\n");
