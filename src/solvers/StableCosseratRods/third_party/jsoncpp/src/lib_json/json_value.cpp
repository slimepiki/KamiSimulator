...
void Value::resize(ArrayIndex newSize) {
    ...
    if (newSize == 0)
        ...
        for (ArrayIndex i = oldSize; i < newSize; ++i) (*this)[i];
    else {
...
void Value::Comments::set(CommentPlacement slot, String comment) {
    if (slot >= CommentPlacement::numberOfCommentPlacement) return;
    if (!ptr_) ptr_ = std::unique_ptr<Array>(new Array());
...
void Value::setComment(String comment, CommentPlacement placement) {
    ...
    JSON_ASSERT_MESSAGE(comment.empty() || comment[0] == '/', "in Json::Value::setComment(): Comments must start with /");
...
Path::Path(const String& path, const PathArgument& a1, const PathArgument& a2, const PathArgument& a3, const PathArgument& a4,
