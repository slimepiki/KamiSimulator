...
struct LBVH::thrustImpl {
    // Thrust can cause undefined behaviours when its vector size is too large. Thus, you may have to implement a thrust-free version if you want to
    // simulate hair-hair collisions.
    ...
    thrust::device_vector<uint32_t>
        d_leafParents;                    // Parent ID for each leaf. MSB is used to indicate whether this is a left or right child of said parent.
...
__device__ inline int commonUpperBits(const uint64_t lhs, const uint64_t rhs) { return ::__clzll(lhs ^ rhs); }

// Merges morton code with its index to output a sorted unique 64-bit key.
__device__ inline uint64_t mergeIdx(const uint32_t code, const int idx) { return ((uint64_t)code << 32ul) | (uint64_t)idx; }

__device__ inline ivec2 determineRange(uint32_t const* mortonCodes, const uint32_t numObjs, uint32_t idx) {
    // This is the root node
    if (idx == 0) return ivec2(0, numObjs - 1);
    ...
    while (t > 0) {
        i = idx + (l + t) * d;
        if (0 <= i && i < numObjs)
            if (commonUpperBits(selfCode, mergeIdx(mortonCodes[i], i)) > minDelta) l += t;
        t >>= 1;
    }

    unsigned int jdx = idx + l * d;
    if (d < 0) hcuSwap(idx, jdx);  // Make sure that idx < jdx
...
__device__ inline uint32_t findSplit(uint32_t const* mortonCodes, const uint32_t first, const uint32_t last) {
    ...
    do {
        ...
        if (middle < last)
            if (commonUpperBits(firstCode, mergeIdx(mortonCodes[middle], middle)) > deltaNode) split = middle;
    } while (stride > 1);
...
__global__ void lbvhBuildInternalKernel(LBVH::node* nodes, uint32_t* leafParents, uint32_t const* mortonCodes, uint32_t const* objIDs, int numObjs) {
    const int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void mergeUpKernel(LBVH::node* nodes, uint32_t* leafParents, Bound<3, float>* aabbs, uint32_t* objIDs, int* flags, int numObjs) {
    const int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void lbvhQueryKernel(ivec2* res, int* resCounter, int maxRes, const LBVH::node* nodes, const uint32_t* objIDs, const uint32_t* queryIDs,
                                const Bound<3, float>* queryAABBs, const int numQueries) {
    const int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    *(stackPtr++) = 0;  // Push
    ...
    while (true) {
        ...
        if (active)
            while (stackPtr != stack) {
                ...
                if (isLeaf) {
                    ...
                    // Internal node
                    if (node.bounds[0].intersects(queryAABB)) *(stackPtr++) = node.leftIdx;  // Push

                    if (node.bounds[1].intersects(queryAABB)) *(stackPtr++) = node.rightIdx;  // Push
                }
            }
        ...
        if (threadIdx.x == 0) sharedGlobalIdx = atomicAdd(resCounter, totalRes);
...
void launchQueryKernel(ivec2* res, int* resCounter, int maxRes, const LBVH::node* nodes, const uint32_t* objIDs, const uint32_t* queryIDs,
                       const Bound<3, float>* queryAABBs, const int numQueries, int stackSize) {
...
#define DISPATCH_QUERY(N)                                                                                                  \
    case N:                                                                                                                \
        lbvhQueryKernel<IGNORE_SELF, N>                                                                                    \
            <<<(numQueries + 127) / 128, 128>>>(res, resCounter, maxRes, nodes, objIDs, queryIDs, queryAABBs, numQueries); \
        break;
    switch (stackSize) {
        default:
            DISPATCH_QUERY(32);
...
LBVH::aabb LBVH::bounds() { return rootBounds; }

__host__ void LBVH::refit() {
    cudaMemset(thrust::raw_pointer_cast(impl->d_flags.data()), 0, sizeof(int) * (numObjs - 1));
    // Go through and merge all the aabbs up from the leaf nodes
    LBVHKernels::mergeUpKernel<<<(numObjs + 255) / 256, 256>>>(
        thrust::raw_pointer_cast(impl->d_nodes.data()), thrust::raw_pointer_cast(impl->d_leafParents.data()), thrust::raw_pointer_cast(impl->d_objs),
        thrust::raw_pointer_cast(impl->d_objIDs.data()), thrust::raw_pointer_cast(impl->d_flags.data()), numObjs);

    checkCudaErrors(cudaGetLastError());
}
__host__ void LBVH::compute(aabb* devicePtr, size_t size) {
    // impl.reset(new thrustImpl());
    ...
    checkImplFirstFlag(10);
    impl->d_objIDs.resize(numObjs);
    impl->d_leafParents.resize(numObjs);
    impl->d_nodes.resize(numObjs);
    impl->d_flags.resize(numObjs);

    // Compute the bounding box for the whole scene so we can assign morton codes
    rootBounds = aabb();
    rootBounds = thrust::reduce(impl->d_objs, impl->d_objs + numObjs, rootBounds, [] __host__ __device__(const aabb& lhs, const aabb& rhs) {
        auto b = lhs;
        b.absorb(rhs);
        return b;
    });
    ...
    LBVHKernels::mortonKernel<float><<<(numObjs + 255) / 256, 256>>>(devicePtr, thrust::raw_pointer_cast(impl->d_morton.data()),
    ...
    LBVHKernels::lbvhBuildInternalKernel<<<(numObjs + 255) / 256, 256>>>(
        thrust::raw_pointer_cast(impl->d_nodes.data()), thrust::raw_pointer_cast(impl->d_leafParents.data()),
        thrust::raw_pointer_cast(impl->d_morton.data()), thrust::raw_pointer_cast(impl->d_objIDs.data()), numObjs);
...
//    LBVHKernels::launchQueryKernel<false>(d_res, d_counter, resSize, thrust::raw_pointer_cast(impl->d_nodes.data()),
//                                          thrust::raw_pointer_cast(impl->d_objIDs.data()), thrust::raw_pointer_cast(other->impl->d_objIDs.data()),
//                                          thrust::raw_pointer_cast(other->impl->d_objs), numQuery, maxStackSize);

//    checkCudaErrors(cudaGetLastError());
//    return std::min((size_t)impl->d_flags[0], resSize);
//}

__host__ size_t LBVH::query(ivec2* d_res, size_t maxRes) const {
    // Borrow the flags array for the counter
    int* d_counter = (int*)thrust::raw_pointer_cast(impl->d_flags.data());
    cudaMemset(d_counter, 0, sizeof(int));
    ...
    LBVHKernels::launchQueryKernel<true>(d_res, d_counter, maxRes, thrust::raw_pointer_cast(impl->d_nodes.data()),
                                         thrust::raw_pointer_cast(impl->d_objIDs.data()), thrust::raw_pointer_cast(impl->d_objIDs.data()),
                                         thrust::raw_pointer_cast(impl->d_objs), numQuery, maxStackSize);

    checkCudaErrors(cudaGetLastError());
    // printf("flags: %zu\n", (size_t)impl->d_flags[0]);

    return std::min((size_t)impl->d_flags[0], maxRes);
}
...
Kitten::Bound<3, float> lbvhCheckAABBMerge(thrust::host_vector<LBVH::node>& nodes, uint32_t idx) {
    auto node = nodes[idx];
    if (!(node.leftIdx >> 31)) {
...
ivec2 lbvhCheckIndexMerge(thrust::host_vector<LBVH::node>& nodes, uint32_t idx, int numObjs) {
    bool isLeaf = idx >> 31;
    idx &= 0x7FFFFFFF;
    ...
    // Check if they match
    if (left != range) printf("Error: Index range mismatch\n");
...
void LBVH::bvhSelfCheck() const {
    ...
    // Check sizes
    if (nodes.size() != numObjs - 1 || morton.size() != numObjs) printf("Error: Incorrect memory sizes\n");

    // Check morton codes
    for (size_t i = 1; i < numObjs; i++)
        if (morton[i - 1] > morton[i]) printf("Bad morton code ordering\n");
    ...
    for (size_t i = 1; i < numObjs - 1; i++) {
        ...
        if ((isRight ? parent.rightIdx : parent.leftIdx) != i) printf("Error: Child node has incorrect parent\n");
    }
    ...
    for (size_t i = 0; i < numObjs; i++) {
        ...
        if ((isRight ? parent.rightIdx : parent.leftIdx) != (i | 0x80000000)) printf("Error: Leaf node has incorrect parent\n");
    }
    ...
    if (numVisited != numObjs * 2 - 1)
        printf("Error: Not all nodes are accessible from the root. Only %d/%zu nodes are found.\n", numVisited, 2 * numObjs - 1);
    if (maxStackSize != maxSize) printf("Error: Max stack size mismatch. Stored %d, found %d\n", maxStackSize, maxSize);
...
void LBVH::checkImplFirstFlag(int id) const {
    if (id)
        printf("Check Impl Flags... (id = %d)\n", id);
    else
        printf("Check Impl Flags... \n");
    printf("flag[0]:\t\t %zu\n", (size_t)impl->d_flags[0]);
}

void LBVH::reset() { impl.release(); }
...
void testLBVH() {
    ...
    printf("%zu collision pairs found on GPU.\n", res.size());
    // printf("GPU:\n");
