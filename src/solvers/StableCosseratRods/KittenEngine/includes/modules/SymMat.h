...
struct hess3 {
    ...
    KITTEN_FUNC_DECL hess3(float d) {
        for (int i = 0; i < 6; ++i) dat[i] = d;
    }
    KITTEN_FUNC_DECL hess3(const hess3& m) {
        for (int i = 0; i < 6; ++i) dat[i] = m.dat[i];
    }
    ...
    KITTEN_FUNC_DECL float& operator[](const int i) { return dat[i]; }
    KITTEN_FUNC_DECL const float& operator[](const int i) const { return dat[i]; }
    ...
    KITTEN_FUNC_DECL hess3 operator+(const hess3& m) const {
        hess3 r;
        for (int i = 0; i < 6; ++i) r.dat[i] = dat[i] + m.dat[i];
        return r;
    }
    KITTEN_FUNC_DECL hess3 operator-(const hess3& m) const {
        hess3 r;
    ...
    KITTEN_FUNC_DECL hess3 operator/(const float s) const {
        float invS = 1.0f / s;
        hess3 r;
        for (int i = 0; i < 6; ++i) r.dat[i] = dat[i] * invS;
        return r;
    }
...
struct hess4 {
    ...
    KITTEN_FUNC_DECL hess4(float d) {
        for (int i = 0; i < 10; ++i) dat[i] = d;
    }
    KITTEN_FUNC_DECL hess4(const hess4& m) {
        for (int i = 0; i < 10; ++i) dat[i] = m.dat[i];
    }
    ...
    KITTEN_FUNC_DECL hess4 operator+(const hess4& m) const {
        hess4 r;
        for (int i = 0; i < 10; ++i) r.dat[i] = dat[i] + m.dat[i];
        return r;
    }
    KITTEN_FUNC_DECL hess4 operator-(const hess4& m) const {
        hess4 r;
    ...
    KITTEN_FUNC_DECL hess4 operator*(const float s) const {
        hess4 r;
        for (int i = 0; i < 10; ++i) r.dat[i] = dat[i] * s;
        return r;
    }
    KITTEN_FUNC_DECL hess4 operator/(const float s) const {
        float invS = 1.0f / s;
        hess4 r;
        for (int i = 0; i < 10; ++i) r.dat[i] = dat[i] * invS;
        return r;
    }
    KITTEN_FUNC_DECL hess4 operator-() const {
        hess4 r;
        for (int i = 0; i < 10; ++i) r.dat[i] = -dat[i];
        return r;
    }
...
struct SymMat {
    ...
    constexpr KITTEN_FUNC_DECL SymMat(const SymMat& other) : data{} {
        for (int i = 0; i < DATA_LEN; ++i) data[i] = other.data[i];
    }
    ...
    constexpr KITTEN_FUNC_DECL explicit SymMat(const SymMat<M, U>& other) {
        ...
        // Copy diaganol
        for (int i = 0; i < K; i++) data[i] = other.data[i];
        // Copy upper triangle
        constexpr int KLim = (K * (K + 1)) / 2 - K;
        for (int i = 0; i < KLim; i++) data[N + i] = other.data[M + i];
        // Zero out the rest
        for (int i = K + KLim; i < DATA_LEN; i++) data[i] = T(0);
    }
    ...
    constexpr KITTEN_FUNC_DECL explicit operator mat_type() const {
        mat_type m;
        for (int i = 0; i < N; i++) m[i][i] = data[i];
    ...
    constexpr KITTEN_FUNC_DECL const vec_type col(const int i) const {
        ...
        for (int j = 0; j < i; j++) v[j] = data[j + N + (i * (i - 1)) / 2];
        // Grab diagonal
        v[i] = data[i];
        // Grab after diagonal
        for (int j = i + 1; j < N; j++) v[j] = data[i + N + (j * (j - 1)) / 2];
    ...
    constexpr KITTEN_FUNC_DECL SymMat<N, T> operator+(const SymMat<N, T>& other) const {
        SymMat<N, T> result;
        for (int i = 0; i < DATA_LEN; i++) result.data[i] = data[i] + other.data[i];
        return result;
    }
    ...
    constexpr KITTEN_FUNC_DECL SymMat<N, T> operator*(const T scalar) const {
        SymMat<N, T> result;
        for (int i = 0; i < DATA_LEN; i++) result.data[i] = data[i] * scalar;
        return result;
    }
    ...
    constexpr KITTEN_FUNC_DECL SymMat<N, T> operator/(const T scalar) const {
        ...
        SymMat<N, T> result;
        for (int i = 0; i < DATA_LEN; i++) result.data[i] = data[i] * invScalar;
        return result;
    }

    constexpr KITTEN_FUNC_DECL SymMat<N, T> operator*(const SymMat<N, T>& other) const {
        SymMat<N, T> result;
        for (int i = 0; i < N; i++) result[i] = dot(col(i), other.col(i));
        for (int i = 1; i < N; i++)
            for (int j = 0; j < i; j++) result[N + j + (i * (i - 1)) / 2] = dot(col(i), other.col(j));
        return result;
    }

    constexpr KITTEN_FUNC_DECL vec_type operator*(const vec_type& v) const {
        vec_type result = v[0] * col(0);
        for (int i = 1; i < N; i++) result += v[i] * col(i);
        return result;
    }

    constexpr KITTEN_FUNC_DECL SymMat<N, T> operator-() const {
        SymMat<N, T> result;
        for (int i = 0; i < DATA_LEN; i++) result.data[i] = -data[i];
        return result;
    }

    constexpr KITTEN_FUNC_DECL SymMat<N, T>& operator+=(const T scaler) {
        for (int i = 0; i < N; i++) data[i] += scaler;
        return *this;
    }
    ...
    constexpr KITTEN_FUNC_DECL SymMat<N, T>& operator*=(const T scalar) {
        for (int i = 0; i < DATA_LEN; i++) data[i] *= scalar;
        return *this;
    }

    constexpr KITTEN_FUNC_DECL SymMat<N, T>& operator/=(const T scalar) {
        T invScalar = 1 / scalar;
        for (int i = 0; i < DATA_LEN; i++) data[i] *= invScalar;
        return *this;
    }
    ...
    constexpr KITTEN_FUNC_DECL T& operator[](const int i) { return data[i]; }

    constexpr KITTEN_FUNC_DECL const T& operator[](const int i) const { return data[i]; }

    constexpr KITTEN_FUNC_DECL static const SymMat<N, T> outer(const vec_type v) {
        SymMat<N, T> result(0);
        for (int i = 0; i < N; i++) result[i] = v[i] * v[i];
        for (int i = 1; i < N; i++)
            for (int j = 0; j < i; j++) result[N + j + (i * (i - 1)) / 2] = v[i] * v[j];
        return result;
    }
};
