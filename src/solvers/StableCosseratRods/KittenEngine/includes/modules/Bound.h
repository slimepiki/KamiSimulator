...
struct Bound {
    ...
    KITTEN_FUNC_DECL inline vec<dim, Real, defaultp> center() const { return (min + max) * 0.5f; }
    ...
    KITTEN_FUNC_DECL inline bool intersects(const Bound<dim, Real>& b) const {
        return !(glm::any(lessThanEqual(max, b.min)) || glm::any(greaterThanEqual(min, b.max)));
    }
    ...
    KITTEN_FUNC_DECL inline vec<dim, Real, defaultp> normCoord(const vec<dim, Real, defaultp>& pos) const { return (pos - min) / (max - min); }

    KITTEN_FUNC_DECL inline vec<dim, Real, defaultp> interp(const vec<dim, Real, defaultp>& coord) const {
        vec<dim, Real, defaultp> pos;
        vec<dim, Real, defaultp> diff = max - min;
        for (int i = 0; i < dim; i++) pos[i] = min[i] + diff[i] * coord[i];
        return pos;
    }
};
...
struct Bound<1, Real> {
    ...
    KITTEN_FUNC_DECL inline Real center() const { return (min + max) * 0.5f; }
    ...
    KITTEN_FUNC_DECL inline bool contains(Real point) const { return min <= point && point <= max; }

    KITTEN_FUNC_DECL inline bool contains(const Bound<1, Real>& b) const { return min <= b.min && b.max <= max; }

    KITTEN_FUNC_DECL inline bool intersects(const Bound<1, Real>& b) const { return max > b.min && min < b.max; }
    ...
    KITTEN_FUNC_DECL inline Real volume() const { return max - min; }

    KITTEN_FUNC_DECL inline Real normCoord(Real pos) const { return (pos - min) / (max - min); }

    KITTEN_FUNC_DECL inline Real interp(Real coord) const { return min + (max - min) * coord; }
};
