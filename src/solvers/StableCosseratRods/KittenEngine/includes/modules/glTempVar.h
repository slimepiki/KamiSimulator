...
#define GEN_INT_SPEC_PARAM(key, setfunc, param)  \
    ...
    class glTempVar<key> {                       \
       public:                                   \
        ...
        ~glTempVar() { setfunc(param, oldVal); } \
    };
...
#define GEN_VEC2_SPEC(key, setfunc)                            \
    ...
    class glTempVar<key> {                                     \
       public:                                                 \
        ...
        glTempVar(float a, float b) : glTempVar(vec2(a, b)) {} \
        ~glTempVar() { setfunc(oldVal.x, oldVal.y); }          \
    };
...
#define GEN_VEC4_SPEC(key, setfunc)                                                    \
    ...
    class glTempVar<key> {                                                             \
       public:                                                                         \
        ...
        glTempVar(float a, float b, float c, float d) : glTempVar(vec4(a, b, c, d)) {} \
        ~glTempVar() { setfunc(oldVal.x, oldVal.y, oldVal.z, oldVal.w); }              \
    };
...
#define GEN_IVEC4_SPEC(key, setfunc)                                            \
    ...
    class glTempVar<key> {                                                      \
       public:                                                                  \
        ...
        glTempVar(int a, int b, int c, int d) : glTempVar(ivec4(a, b, c, d)) {} \
        ~glTempVar() { setfunc(oldVal.x, oldVal.y, oldVal.z, oldVal.w); }       \
    };
...
#define GEN_BVEC4_SPEC(key, setfunc)                                                \
    ...
    class glTempVar<key> {                                                          \
       public:                                                                      \
        ...
        glTempVar(bool a, bool b, bool c, bool d) : glTempVar(bvec4(a, b, c, d)) {} \
        ~glTempVar() { setfunc(oldVal[0], oldVal[1], oldVal[2], oldVal[3]); }       \
    };
...
#define GEN_BLEND_SPEC(key)                                                               \
    ...
    class glTempVar<key> {                                                                \
       public:                                                                            \
        ...
        glTempVar(GLenum sfactor, GLenum dfactor) : glTempVar(ivec2(sfactor, dfactor)) {} \
        ~glTempVar() { glBlendFunc(oldVal.x, oldVal.y); }                                 \
    };
...
#define GEN_STENCIL_OP_SPEC(key)                                                                           \
    ...
    class glTempVar<key> {                                                                                 \
       public:                                                                                             \
        ...
        glTempVar(GLenum sfail, GLenum dpfail, GLenum dppass) : glTempVar(ivec3(sfail, dpfail, dppass)) {} \
        ~glTempVar() { glStencilOp(oldVal.x, oldVal.y, oldVal.z); }                                        \
    };
...
#define GEN_STENCIL_FUNC_SPEC(key)                                                            \
    ...
    class glTempVar<key> {                                                                    \
       public:                                                                                \
        ...
        glTempVar(GLenum func, GLint ref, GLuint mask) : oldVal0(0), oldVal1(0), oldVal2(0) { \
        ...
        ~glTempVar() { glStencilFunc(oldVal0, oldVal1, oldVal2); }                            \
    };
