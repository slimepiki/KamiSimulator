...
#include "../../../../../../extern/glm/glm/gtx/compatibility.hpp"
...
KITTEN_FUNC_DECL inline int flatIdx(vec<s, int, defaultp> i, vec<s, int, defaultp> dim) {
    int idx = i[s - 1];
    for (int k = s - 2; k >= 0; k--) idx = i[k] + dim[k] * idx;
    return idx;
}
...
KITTEN_FUNC_DECL inline mat4 TRSMat(vec3 t, float r, vec2 s) {
    mat4 mat(1);
    mat = glm::rotate(mat, -r, vec3(0, 0, 1));
    mat = glm::scale(mat, vec3(s, 1));
...
KITTEN_FUNC_DECL inline mat4 TRSMat(vec2 t, float r, vec2 s) {
    ...
    mat = glm::rotate(mat, -r, vec3(0, 0, 1));
    mat = glm::scale(mat, vec3(s, 1));
    mat = glm::translate(mat, vec3(-t, 0));
    return mat;
}
...
KITTEN_FUNC_DECL inline T min(vec<s, T, defaultp> v) {
    T n = v[0];
    for (int i = 1; i < s; i++) n = glm::min(n, v[i]);
    return n;
}
...
KITTEN_FUNC_DECL inline vec<s, T, defaultp> min(vec<s, T, defaultp> v, T num) {
    vec<s, T, defaultp> r;
    for (int i = 0; i < s; i++) r[i] = glm::min(num, v[i]);
    return r;
}
...
KITTEN_FUNC_DECL inline T max(vec<s, T, defaultp> v) {
    T n = v[0];
    for (int i = 1; i < s; i++) n = glm::max(n, v[i]);
    return n;
}
...
KITTEN_FUNC_DECL inline vec<s, T, defaultp> max(vec<s, T, defaultp> v, T num) {
    vec<s, T, defaultp> r;
    for (int i = 0; i < s; i++) r[i] = glm::max(num, v[i]);
    return r;
}
...
KITTEN_FUNC_DECL inline mat<s, c, T, defaultp> mix(mat<s, c, T, defaultp> a, mat<s, c, T, defaultp> b, T t) {
    ...
    for (int i = 0; i < s; i++) m[i] = mix(a[i], b[i], t);
    return m;
}
...
KITTEN_FUNC_DECL inline T norm(mat<s, s, T, defaultp> m) {
    T d = 0;
    for (int i = 0; i < s; i++) d += length(m[i]);
    return d;
}
...
KITTEN_FUNC_DECL inline T ferbNorm(mat<s, s, T, defaultp> m) {
    T d = 0;
    for (int i = 0; i < s; i++) d += length2(m[i]);
    return glm::sqrt(d);
}

KITTEN_FUNC_DECL inline vec3 reflect(vec3 v, vec3 norm) { return v - 2 * dot(norm, v) * norm; }

KITTEN_FUNC_DECL inline vec2 clampLen(vec2 v, float l) {
    float len = v.x * v.x + v.y * v.y;
    if (len > l * l) return v / sqrtf(len) * l;
    return v;
}
...
KITTEN_FUNC_DECL inline mat<3, 3, T, defaultp> abT(vec<3, T, defaultp> a, vec<3, T, defaultp> b) {
    return mat<3, 3, T, defaultp>(b.x * a.x, b.y * a.x, b.z * a.x, b.x * a.y, b.y * a.y, b.z * a.y, b.x * a.z, b.y * a.z, b.z * a.z);
}
...
KITTEN_FUNC_DECL inline mat4 rotateView(vec3 dir) { return mat4(transpose(orthoBasisZ(normalize(-dir)))); }

KITTEN_FUNC_DECL inline int numBatches(int n, int batchSize) { return (n - 1) / batchSize + 1; }

KITTEN_FUNC_DECL inline int numBatches(size_t n, int batchSize) { return numBatches((int)n, batchSize); }
...
KITTEN_FUNC_DECL inline bool lineHasInt(vec2 s) { return s.x >= 0 && s.x <= 1 && s.y >= 0 && s.y <= 1; }
...
KITTEN_FUNC_DECL inline int wrap(int i, int period) { return (i + period) % period; }

// Returns the distance between a and b modulo len
KITTEN_FUNC_DECL inline int cyclicDist(int a, int b, int len) { return ((b - a) + len / 2 + len) % len - len / 2; }
...
inline void print(int x, const char* format = "%d") { printf(format, x); }
...
KITTEN_FUNC_DECL inline vec3 baryCoord(const mat4x3& p) { return baryCoord(*(mat3*)&p, p[3]); }
...
KITTEN_FUNC_DECL inline mat<3, 3, T, defaultp> crossMatrix(vec<3, T, defaultp> v) {
    return mat<3, 3, T, defaultp>(0, v.z, -v.y, -v.z, 0, v.x, v.y, -v.x, 0);
}

KITTEN_FUNC_DECL inline ivec3 getCell(vec3 pos, float h) { return glm::ceil(pos / h); }

KITTEN_FUNC_DECL inline int cantorHashCombine(int a, int b) { return ((a + b) * (a + b + 1)) / 2 + b; }
...
//    mat<3, 3, T, defaultp> A(U[0][0] * V[0][0] + U[1][0] * V[1][0] + U[2][0] * V[2][0], U[0][1] * V[0][0] + U[1][1] * V[1][0] + U[2][1] * V[2][0],
//                             U[0][2] * V[0][0] + U[1][2] * V[1][0] + U[2][2] * V[2][0],

//                             U[0][0] * V[0][1] + U[1][0] * V[1][1] + U[2][0] * V[2][1], U[0][1] * V[0][1] + U[1][1] * V[1][1] + U[2][1] * V[2][1],
//                             U[0][2] * V[0][1] + U[1][2] * V[1][1] + U[2][2] * V[2][1],

//                             U[0][0] * V[0][2] + U[1][0] * V[1][2] + U[2][0] * V[2][2], U[0][1] * V[0][2] + U[1][1] * V[1][2] + U[2][1] * V[2][2],
...
//     for (int i = 0; i < 3; i++) val[i] = data[i * stride];
//     return val;
// }

template <int s, typename T>
// Performs uniform catmull rom spline interpolation with t from 0 to 1 in between p1 and p2
KITTEN_FUNC_DECL inline vec<s, T, defaultp> cmrSpline(vec<s, T, defaultp> p0, vec<s, T, defaultp> p1, vec<s, T, defaultp> p2, vec<s, T, defaultp> p3,
                                                      T t) {
...
template <int s, typename T>
KITTEN_FUNC_DECL inline vec<s, T, defaultp> cmrSplineTangent(vec<s, T, defaultp> p0, vec<s, T, defaultp> p1, vec<s, T, defaultp> p2,
                                                             vec<s, T, defaultp> p3, T t) {
    return 0.5f * ((-(t - 1) * (3 * t - 1)) * p0 + (t * (9 * t - 10)) * p1 + -(t - 1) * (9 * t + 1) * p2 + t * (3 * t - 2) * p3);
}

KITTEN_FUNC_DECL inline double mix(double a, double b, double t) { return a * (1.0 - t) + b * t; }
}  // namespace Kitten
...
struct hash<glm::vec<N, int, glm::defaultp>> {
    std::size_t operator()(const glm::vec<N, int, glm::defaultp>& v) const {
        size_t h = 0x9e3779b9;
        for (int i = 0; i < N; i++) h = v[i] ^ (h + 0x9e3779b9 + (v[i] << 6) + (v[i] >> 2));
        return h;
    }
};
...
struct hash<glm::vec<N, glm::i64, glm::defaultp>> {
    std::size_t operator()(const glm::vec<N, int, glm::defaultp>& v) const {
        size_t h = 0x9e3779b9;
        for (int i = 0; i < N; i++) h = v[i] ^ (h + 0x9e3779b9llu + (v[i] << 6) + (v[i] >> 2));
        return h;
    }
};
