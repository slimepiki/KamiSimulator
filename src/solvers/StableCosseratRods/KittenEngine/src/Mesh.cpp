...
int Mesh::hashTriangles() {
    ...
    for (int i : indices) hash = boostHashCombine(hash, i);
    for (auto& v : vertices) {
        for (size_t k = 0; k < sizeof(Vertex) / sizeof(int); k++) hash = boostHashCombine(hash, ((int*)&v)[k]);
    }
    return hash;
}
...
void Mesh::setFromLine(vector<vec3>& points) {
    ...
    for (size_t i = 0; i < points.size(); i++) vertices[i] = {points[i]};
...
void Mesh::calculateBounds() {
    ...
    for (size_t i = 0; i < vertices.size(); i++) bounds.absorb(vertices[i].pos);
}
...
void Mesh::writeOBJ(string p) {
    ...
    if (file) {
        ...
        for (size_t i = 0; i < indices.size() / 3; i++) {
            ivec3 v = ivec3(indices[3 * i + 0], indices[3 * i + 1], indices[3 * i + 2]) + 1;
    ...
    for (int ix = 0; ix < samplesPerAxis; ix++)
        for (int iz = 0; iz < samplesPerAxis; iz++) {
            ...
            for (size_t i = 0; i < indices.size(); i += 3) {
                ...
                dvec3 bary;
                double t;
                if (robustRayTriInt(ori, dir, tri, bary, t)) sortedInts.push_back((float)t);
            }

            if (sortedInts.size() & 1) throw std::runtime_error("Mesh not closed!");
...
Mesh* loadMeshFrom(std::filesystem::path path) {
    ...
    for (size_t i = 0; i < scene->mNumMaterials; i++) {
        ...
        if (AI_SUCCESS != aim->Get(AI_MATKEY_OPACITY, mat->props.col.a)) mat->props.col.a = 1;
        ...
        if (node->mNumMeshes) {
            ...
            for (size_t i = 0; i < node->mNumMeshes; i++) {
                ...
                for (size_t j = 0; j < aim->mNumFaces; j++) {
                    ...
                    for (unsigned int k = 0; k < face.mNumIndices; k++) mesh->indices.push_back(face.mIndices[k] + startIndex);
                }
...
size_t TetMesh::numTet() { return tetIndices.size() / 4; }
...
void TetMesh::writeMSH(string p) {
    ...
    if (file) {
        ...
        for (size_t i = 1; i <= vertices.size(); i++) fprintf(file, "%zd\n", i);
        ...
        for (size_t i = 0; i < numTet(); i++) {
            fprintf(file, "%zd %d %d %d %d\n", i + 1, tetIndices[4 * i + 0] + 1, tetIndices[4 * i + 1] + 1, tetIndices[4 * i + 2] + 1,
                    tetIndices[4 * i + 3] + 1);
        }
...
void TetMesh::flipInverted() {
    for (size_t i = 0; i < tetIndices.size(); i += 4) {
        ...
        float v = determinant(mat3(vertices[i1].pos - vertices[i0].pos, vertices[i2].pos - vertices[i0].pos, vertices[i3].pos - vertices[i0].pos));
...
void TetMesh::regenSurface() {
    ...
    for (size_t i = 0; i < tetIndices.size(); i += 4) {
        tri tris[4]{
            make_tuple(tetIndices[i + 0], tetIndices[i + 2], tetIndices[i + 1]), make_tuple(tetIndices[i + 0], tetIndices[i + 1], tetIndices[i + 3]),
            make_tuple(tetIndices[i + 1], tetIndices[i + 2], tetIndices[i + 3]), make_tuple(tetIndices[i + 2], tetIndices[i + 0], tetIndices[i + 3])};
...
void TetMesh::writeTetsOBJ(string p) {
    ...
    if (file) {
        ...
        fprintf(file, "# %zd vertices\n\no mesh\n", vertices.size());
        for (size_t i = 0; i < numTet(); i++) {
            ivec4 v = ivec4(tetIndices[4 * i + 0], tetIndices[4 * i + 1], tetIndices[4 * i + 2], tetIndices[4 * i + 3]) + 1;
