...
namespace Kitten {
std::vector<std::string> includePaths = {"shaders"};
...
bool endsWith(string const& str, string const& ending) {
    if (str.length() >= ending.length()) return (0 == str.compare(str.length() - ending.length(), ending.length(), ending));
    return false;
}
...
string loadTextWithIncludes(const string root, string path, set<string>& includedSet) {
    ...
    if (!file.exists()) file = fs::directory_entry(path);
    if (!file.exists()) {
    ...
    if (includedSet.count(path)) return "";
