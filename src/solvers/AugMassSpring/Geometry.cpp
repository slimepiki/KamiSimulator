...
Triangle::Triangle() : fa(-1), fb(-1), fc(-1), a(), b(), c(), n(), center(), perVertexNormal(false), selected(0) {}

Triangle::Triangle(const vec3 &va, const vec3 &vb, const vec3 &vc)
    : fa(-1), fb(-1), fc(-1), a(va), b(vb), c(vc), n(), center(), perVertexNormal(false), selected(0) {
    center = (a + b + c) / 3.0f;
}

Triangle::Triangle(const vec3 &va, const vec3 &vb, const vec3 &vc, const vec3 &vna, const vec3 &vnb, const vec3 &vnc)
    : fa(-1), fb(-1), fc(-1), a(va), b(vb), c(vc), n(), center(), perVertexNormal(false), na(vna), nb(vnb), nc(vnc), selected(0) {
    center = (a + b + c) / 3.0f;
}

Triangle::Triangle(const vec3 &va, const vec3 &vb, const vec3 &vc, const vec3 &vna, const vec3 &vnb, const vec3 &vnc, const vec2 &tan,
                   const vec2 &tbn, const vec2 &tcn)
    : fa(-1),
      fb(-1),
      fc(-1),
      a(va),
...
Triangle::Triangle(const vec3 &va, const vec3 &vb, const vec3 &vc, const vec3 &vn, int ifa, int ifb, int ifc)
    : fa(ifa), fb(ifb), fc(ifc), a(va), b(vb), c(vc), n(vn), perVertexNormal(false), selected(0) {
    center = (a + b + c) / 3.0f;
}

Triangle::Triangle(const vec3 &va, const vec3 &vb, const vec3 &vc, const vec3 &vn, const vec3 &vna, const vec3 &vnb, const vec3 &vnc, int ifa,
                   int ifb, int ifc)
    : fa(ifa), fb(ifb), fc(ifc), a(va), b(vb), c(vc), n(vn), perVertexNormal(false), na(vna), nb(vnb), nc(vnc), selected(0) {
    center = (a + b + c) / 3.0f;
}
...
bool Triangle::intersect(const vec3 &rayStart, const vec3 &rayDir, float &t) const { return intersect(rayStart, rayDir, a, b, c, t); }
...
bool Triangle::intersect(const vec3 &rayStart, const vec3 &rayDir, vec3 &point) const { return intersect(rayStart, rayDir, a, b, c, point); }
...
bool Triangle::intersect(vec3 rayStart, vec3 rayDir, vec3 v0, vec3 v1, vec3 v2, vec3 &point) const {
    ...
    vec3 v = v2 - v0;
    vec3 n = cross(u, v);
    if (n == vec3(0, 0, 0)) return false;  // triangle is a segment or point
    ...
    float r = a / b;
    if (r < 0.0) return false;  // no intersect
...
bool Triangle::inside(const vec3 &p) const {
    ...
    // Test sign of r
    if (dot(vCrossW, vCrossU) < 0) return false;

    vec3 uCrossW = cross(u, w);
    vec3 uCrossV = cross(u, v);

    // Test sign of t
    if (dot(uCrossW, uCrossV) < 0) return false;
...
int Triangle::getNeighbourCrossingIntersection(const vec3 &origin, const vec3 &dir, float *tOut, vec3 *edgeNormal, float maxDist) {
    ...
    *tOut = minT;
    ...
    for (size_t i = 0; i < neighbors.size(); i++) {
        ...
        // Check if edge vertices match -> works, but not a good way, todo better, implicitly by index
        if ((n.s == v[minIndex] && n.t == v[(minIndex + 1) % 3]) || (n.t == v[minIndex] && n.s == v[(minIndex + 1) % 3])) {
            return i;
        }
    }
...
int Triangle::shareCommonEdge(const Triangle &t, vec3 &s, vec3 &e) {
    ...
    if (fa == t.fa || fa == t.fb || fa == t.fc || lengthSq(t.a - a) < eps || lengthSq(t.b - a) < eps || lengthSq(t.c - a) < eps) asim = true;

    if (fb == t.fa || fb == t.fb || fb == t.fc || lengthSq(t.a - b) < eps || lengthSq(t.b - b) < eps || lengthSq(t.c - b) < eps) bsim = true;

    if (fc == t.fa || fc == t.fb || fc == t.fc || lengthSq(t.a - c) < eps || lengthSq(t.b - c) < eps || lengthSq(t.c - c) < eps) csim = true;
...
Plane::Plane(vec3 &v1, vec3 &v2, vec3 &v3) { set3Points(v1, v2, v3); }
...
float Plane::distance(vec3 p) { return (d + normal.dot(p)); }
...
vec3 Frustum::getVertexP(vec3 &normal, vec3 &min, vec3 &max) {
    ...
    if (normal.x > 0) res.x += (max.x - min.x);

    if (normal.y > 0) res.y += (max.y - min.y);

    if (normal.z > 0) res.z += (max.z - min.z);
...
void Picking::getPickingRay(const Transform &trans, const float fov, const float ncp, const float window_width, const float window_height,
                            float mouse_x, float mouse_y, vec3 &rayPos, vec3 &rayDir) {
    ...
    float vLength = float(tan(rad / 2.0) * ncp);
...
Spline::Spline(Config conf) : m_phantomStart(), m_phantomEnd(), m_deltaT(0.0f), m_config(conf) {}
...
void Spline::addPoint(const vec3 &v) {
    ...
    if (m_points.size() >= 3) {
        ...
        uint size = (uint)m_points.size() - 1;
...
vec3 Spline::interpolatedPoint(float t, Config conf) {
    ...
    if (conf == CATMULL_ROM) return Spline::catmullRomInterpolation(c1, v1, v2, c2, lt);
    ...
    if (conf == HERMITE) return Spline::hermiteInterpolation(c1, v1, v2, c2, lt);
    if (conf == KOCHANEK_BARTEL) return Spline::kochanekBartelInterpolation(c1, v1, v2, c2, lt);
    if (conf == ROUNDED_CATMULL_ROM) return Spline::roundedCatmullRomInterpolation(c1, v1, v2, c2, lt);

    // return LINEAR in default case
    return Spline::linearInterpolation(v1, v2, lt);
}

vec3 Spline::point(int n) const { return m_points[n]; }

int Spline::numPoints() const { return (int)m_points.size(); }
...
vec3 Spline::linearInterpolation(const vec3 &p0, const vec3 &p1, float t) { return p0 * t + (p1 * (1 - t)); }
...
vec3 Spline::cubicInterpolation(const vec3 &p0, const vec3 &p1, const vec3 &p2, const vec3 &p3, float t) {
    float t2;
    vec3 a0, a1, a2, a3;
...
vec3 Spline::bSplineInterpolation(const vec3 &p1, const vec3 &p2, const vec3 &p3, const vec3 &p4, float t) {
    // not working right

    Vector4 a, b, c;

    a.x = (-p1.x + 3.0f * p2.x - 3.0f * p3.x + p4.x) / 6.0f;
    a.y = (3.0f * p1.x - 6.0f * p2.x + 3.0f * p3.x) / 6.0f;
    a.z = (-3.0f * p1.x + 3.0f * p3.x) / 6.0f;
    a.w = (p1.x + 4.0f * p2.x + p3.x) / 6.0f;

    b.x = (-p1.y + 3 * p2.y - 3.0f * p3.y + p4.y) / 6.0f;
    b.y = (3.0f * p1.y - 6.0f * p2.y + 3.0f * p3.y) / 6.0f;
    b.z = (-3.0f * p1.y + 3.0f * p3.y) / 6.0f;
    b.w = (p1.y + 4 * p2.y + p3.y) / 6.0f;

    c.x = (-p1.y + 3.0f * p2.y - 3.0f * p3.y + p4.y) / 6.0f;
    c.y = (3.0f * p1.y - 6.0f * p2.y + 3.0f * p3.y) / 6.0f;
    c.z = (-3.0f * p1.y + 3.0f * p3.y) / 6.0f;
    c.w = (p1.y + 4.0f * p2.y + p3.y) / 6.0f;
...
vec3 Spline::hermiteInterpolation(const vec3 &p0, const vec3 &p1, const vec3 &p2, const vec3 &p3, float t, float tension, float bias) {
    vec3 m0, m1;
    float t2, t3;
    float a0, a1, a2, a3;
...
vec3 Spline::kochanekBartelInterpolation(const vec3 &a, const vec3 &b, const vec3 &c, const vec3 &d, float t, float tension, float bias,
                                         float continuity) {
    // not working right
    ...
    vec3 inTangent =
        ((1.0f - tension) * (1.0f - continuity) * (1.0f + bias)) * 0.5f * ab + ((1.0f - tension) * (1.0f + continuity) * (1.0f - bias)) * 0.5f * cd;
...
int shareCommonEdge(Face &f1, Face &f2) {
    ...
    if (f1.a == f2.a || f1.a == f2.b || f1.a == f2.c) asim = true;

    if (f1.b == f2.a || f1.b == f2.b || f1.b == f2.c) bsim = true;

    if (f1.c == f2.a || f1.c == f2.b || f1.c == f2.c) csim = true;

    int result = -1;

    if (asim && bsim) result = 0;
    if (asim && csim) result = 1;
    if (bsim && csim) result = 2;
